...
plt.figure()
plt.plot(N1,theo_magN,label='<m>_{theo}')
plt.xlabel(r"$N$")
plt.ylabel(r"$m(N)$")
...
for m in range(D):
    ...
    plt.plot(N1,final_M,'o',color=colours[m+1])
...
plt.figure()
plt.plot(h1,theo_magh,label='<m>_{theo}')
plt.xlabel(r"$h$")
plt.ylabel(r"$m(h)$")
...
for m in range(D):
    ...
    plt.plot(h1,final_M,'o',color=colours[m+1])
plt.legend()        
plt.show()  
